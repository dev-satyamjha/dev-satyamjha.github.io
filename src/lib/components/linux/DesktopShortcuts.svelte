<script lang="ts">
	...
	const SHORTCUTS = [
		...
		{ key: 'Super + C', action: 'Compose Transmission', windowId: 'contact' as WindowId },
		{
			...
			custom: () => hyprland.activeWindowId && hyprland.closeWindow(hyprland.activeWindowId)
		},
		{ key: 'Super + 1..5', action: 'Switch Workspaces', custom: () => {} },
		{
			key: 'Super + M',
			action: 'Toggle Maximize Window',
			custom: () => hyprland.activeWindowId && hyprland.toggleMaximize(hyprland.activeWindowId)
		},
		{
			key: 'Super + Space',
			action: 'Toggle Tiling / Floating',
			custom: () => hyprland.toggleTile()
		}
	];
...
<div
	class="fixed bottom-4 left-4 z-10 pointer-events-none hidden lg:block opacity-40 hover:opacity-100 transition-opacity font-mono text-[11px] text-[#a6adc8] space-y-1"
>
	<div class="flex items-center gap-2">
		<span
			class="px-1.5 py-0.5 rounded bg-[#181825] border border-[#313244] text-[#cba6f7] text-[10px]"
			>Super+D</span
		>
...
{#if hyprland.isCheatsheetOpen}
	...
	>
		...
		<div
			class="relative z-10 w-full max-w-lg rounded-3xl border border-[#cba6f7]/40 bg-[#181825] shadow-2xl p-6 sm:p-7 font-mono space-y-5"
		>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
				{#each SHORTCUTS as item}
					<div
						class="p-2.5 rounded-xl bg-[#11111b] border border-[#313244] flex items-center justify-between"
					>
