		...
		<div class="flex items-center gap-2">
			...
			<div>
				<h2 class="font-bold text-sm text-white uppercase tracking-wider">
					Data Vault // Sector 06
				</h2>
				<div class="text-[10px] text-cyan-400/60">
					Quantum Archives & Technical Architecture Articles
				</div>
			</div>
		</div>
		<span
				...
				>
					<div class="space-y-0.5">
						<div class="text-[10px] text-cyan-400/60 uppercase tracking-widest">
							{post.date} &bull; {post.readingTime} &bull; {post.author}
						</div>
					...
					<div class="flex flex-wrap gap-1 pt-1 sm:pt-0">
						{#each post.tags as tag}
							<span
								class="px-2 py-0.5 rounded bg-cyan-950/60 border border-cyan-500/30 text-[9px] text-cyan-300"
							>
								#{tag}
							</span>
						{/each}
