...
<header class="w-full bg-[#030712]/80 backdrop-blur-xl border-b border-cyan-500/30 px-3 sm:px-6 py-2.5 flex items-center justify-between font-mono text-xs text-cyan-300 z-30 select-none">
	<div class="flex items-center gap-3 sm:gap-6">
		...
		<div class="hidden md:flex items-center gap-4 text-[10px] text-cyan-400/70 border-l border-cyan-500/20 pl-4">
			<div><span class="text-white/50">COORDINATES:</span> RA 18h 36m &bull; DEC +38° 47'</div>
			<div><span class="text-white/50">REACTOR:</span> <span class="text-emerald-400">98.6% NOMINAL</span></div>
	...
	<div class="flex items-center gap-3 sm:gap-4 text-xs">
		<div class="hidden sm:block text-[11px] text-cyan-400 bg-cyan-950/40 border border-cyan-500/30 px-2.5 py-0.5 rounded-lg">
			<span class="text-white/50">SD:</span> {stardate}
		</div>
		...
		<div class="relative">
			...
			{#if isWarpMenuOpen}
				...
				>
					<div class="px-2.5 py-1 text-[10px] text-white/40 uppercase tracking-widest font-bold">Warp Coordinates</div>
					<button
						type="button"
